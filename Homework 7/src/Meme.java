public class Meme implements Comparable<Meme>{
	...
	public String toString() {
		String user = getCreator().getUserName();
		return this.backgroundImage + " '" + this.caption + "' " + this.calculateOverallRating() + " [+1: " + calcNumRatings(1) + ", -1: " + calcNumRatings(-1) + "] - created by " + user;
	}
	...
	public int compareTo(Meme other) {
		...
		int backgroundimagecompare = this.backgroundImage.compareTo(other.backgroundImage);
		int ratingcompare = ((Double)calculateOverallRating()).compareTo(other.calculateOverallRating());
		int sharedmemescompare = ((Boolean)getShared()).compareTo(other.getShared());
		
		if (captioncompare == 0) {
			if (backgroundimagecompare == 0) {
				if (ratingcompare == 0) {
					return sharedmemescompare * -1;
				}
				else if (getRatings().length == 0){
					return sharedmemescompare * -1;
				}
				else {
